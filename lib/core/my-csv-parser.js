...
Parser.prototype._online = function (buf, start, end) {
    ...
    var line = buf.slice(start, end);
    this.push(line);
}

Parser.prototype.parseLine = function (row, cb) {
    ...
    for (var i = 0; i < row.length; i++) {
        var isStartingQuote = !isQuoted && row[i] === this.quote;
        var isEndingQuote = isQuoted && row[i] === this.quote && i + 1 <= row.length && row[i + 1] === comma;
        var isEscape = isQuoted && row[i] === this.escape && i + 1 < row.length && row[i + 1] === this.quote;
        ...
        if (row[i] === comma && !isQuoted) {
            cells.push(this._oncell(row, offset, i));
            offset = i + 1;
        }
    }

    if (offset < row.length) cells.push(this._oncell(row, offset, row.length));
    if (row[row.length - 1] === comma) cells.push(this._empty);
    ...
    if (this.strict && cells.length !== this.headers.length) {
        cb(new Error('Row length does not match headers'), null);
    } else {
        cb(null, new this._Row(cells));
    }
}
