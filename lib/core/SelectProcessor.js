...
class SelectProcessor {
    ...
    sendResponse(startRange, endRange) {
        ...
        if (this.isSelectOperation()) {
            ...
            readStream = readStream
                .pipe(this._createFileParser(columnReconciler))
    ...
    _createFileParser(columnReconciler) {
        switch (this.options.source.file_type.toUpperCase()) {
            case 'CSV':
                var parser = csv_parser({
                    delimiter: this.options.source.column_delim,
                    rowDelimiter: this.options.source.row_delim,
                    trim: true
                });
                columnReconciler.doReconcile = (columns) => {
                    parser.options.columns = columns;
                }
                return parser;
